...
} = require('../services/autoMessageTemplateService');
...
exports.getAutoMessages = async (req, res) => {
  try {
    ...
    if (!tenantId) {
      throw createAutoMessageTemplateError(403, 'Tenant context is required');
    }

    const templates = await listAutoMessageTemplates(tenantId, {
      userId: req.user?._id,
    });
...
exports.deleteAutoMessage = async (req, res) => {
  try {
    ...
    const template = await deleteAutoMessageTemplate(req.params?.id, tenantId);
